...
const projectCopy = {
  mgm: "A cinematic AI campaign series developed for Macau MGM.",
  peninsula: "A refined Father’s Day film for a landmark hospitality brand.",
  parknshop: "Fast, flexible retail content designed for weekly communication.",
  octopus: "An urban lifestyle film built around everyday Hong Kong culture.",
  chillgood: "A music-led AI film created for digital entertainment audiences.",
  grams: "Two fashion films exploring colour, movement and monochrome restraint.",
  koisea: "A pair of fashion films moving between open landscape and underground space."
};
...
function workCard(brand, compact = false) {
  const project = brand.projects[0];
  return `
...
function setupWorkCollection({ index = false } = {}) {
  ...
  const render = () => {
    ...
    rail.innerHTML = filtered.map((brand) => workCard(brand, !index)).join("");
...
function renderCase() {
  ...
  root.innerHTML = `
    <article class="case-page">
      ...
      <section class="section-shell next-case">
        <div>
          <p class="micro-label">NEXT PROJECT</p>
          <h2>${escapeHTML(next.name)}</h2>
        </div>
        <a href="case.html?brand=${next.id}">View next case ↗</a>
      </section>
    </article>
  `;
